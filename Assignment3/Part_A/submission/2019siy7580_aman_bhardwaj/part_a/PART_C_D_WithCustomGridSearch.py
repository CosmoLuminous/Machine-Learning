...
def customGridSearch():
    '''Implementation of Custom Grid Search Using OOB Score'''
    now = datetime.now()
    itr = 0
    ...
    print("Start Time =", current_time)
    t0 = time.time()
    for n_esttimator in n_estimator_list:
        for max_feature in max_features_list:
            for min_sample in min_samples_split_list:
                rfc = RandomForestClassifier(bootstrap=True, class_weight=None, criterion='gini',
                       max_depth=None, max_features=max_feature, max_leaf_nodes=None,
                       min_impurity_decrease=0.0, min_impurity_split=None,
                       min_samples_leaf=1, min_samples_split=min_sample,
                       min_weight_fraction_leaf=0.0, n_estimators=n_esttimator,
                       n_jobs=7, oob_score=True, random_state=None,
                       verbose=0, warm_start=False)
                
                rfc.fit(Train_X, Train_Y.ravel())
                oob_scr = rfc.oob_score_
                
                oob_score_list.append(oob_scr)
                paramDict[itr] = {'n_estimator': n_esttimator, 'max_features': max_feature,
                                  'min_samples_split':min_sample}
                itr += 1
                print("Task {} of 125 , Time elapsed: {}Mins".format(itr, round((time.time()-t0)/60, 2)))
    
    max_oob_score = max(oob_score_list)
    argmax_oob = np.argmax(oob_score_list)
    bestParams = paramDict[argmax_oob]
    print("\n@@@@@@@-----CUSTOM GRIDSEARCH RESULTS-----@@@@@@@")
    print("Max OOB Score =", max_oob_score)
    print("Argmax OOB Score =", argmax_oob)
    print("Best Params =", bestParams)
    return bestParams

def getBestParams(TopNResults):
    '''get best params for custom grid search'''
    #TopNResults = 20

    oob_list_copy = copy.deepcopy(oob_score_list)
    argmaxIndices = sorted(range(len(oob_list_copy)), key = lambda sub: oob_list_copy[sub])[-TopNResults:]
    #print(argmaxIndices)
    n_est_list = []
    max_f_list = []
    min_sam_list = []
    for index in argmaxIndices:
        n_est_list.append(paramDict[index]['n_estimator'])
        max_f_list.append(paramDict[index]['max_features'])
        min_sam_list.append(paramDict[index]['min_samples_split'])
    print("Best n_estimator =", mode(n_est_list)[0][0])
    print("Best max_features =", mode(max_f_list)[0][0])
    print("Best min_samples_split =", mode(min_sam_list)[0][0])
    return
